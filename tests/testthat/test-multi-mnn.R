...
test_that("multiModalMNN recapitulates fastMNN", {
    ...
    corrected <- multiModalMNN(exprs_sce, batch=batch, which=character(0), pca.common.args=list(BSPARAM=BSPARAM))
    ...
    corrected2 <- multiModalMNN(copy, batch=batch, which="Spikes", pca.common.args=list(BSPARAM=BSPARAM))
    ...
    corrected3 <- multiModalMNN(
        ...
        which=character(0), pca.common.args=list(BSPARAM=BSPARAM)
    )
    ...
    corrected4 <- multiModalMNN(
        ...
        which="Spikes", pca.common.args=list(BSPARAM=BSPARAM)
    )
...
test_that("multiModalMNN rescales correctly", {
    ...
    suppressWarnings(ref <- multiModalMNN(exprs_sce, batch=batch, which="ADT",
        pca.common.args=list(BSPARAM=BSPARAM)))
...
test_that("multiModalMNN respects Experiment-specific arguments", {
    ...
    corrected <- multiModalMNN(exprs_sce, batch=batch, which="ADT",
        pca.common.args=list(BSPARAM=BSPARAM), pca.main.args=list(subset.row=1:100), 
    ...
    corrected2 <- multiModalMNN(exprs_sce, batch=batch, which="ADT",
        pca.common.args=list(BSPARAM=BSPARAM), 
        pca.main.args=list(subset.row=1:100, correct.all=TRUE), 
        pca.alt.args=list(ADT=list(d=NA)))
