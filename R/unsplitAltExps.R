...
#'
#' @details
#' This is intended for downstream applications that accept a SingleCellExperiment but are not aware of the \code{\link{altExps}} concept.
...
#' However, for more quantitative analyses, it is usually preferable to represent the different modes as separate experiments.
#'
...
#' This is done efficiently via \linkS4class{ConstantMatrix} abstractions to avoid actually creating a dense matrix of \code{NA}s.
#'
...
#'
#' @export
#' @importFrom SingleCellExperiment altExps SingleCellExperiment
unsplitAltExps <- function(sce, prefix.rows=TRUE, prefix.cols=TRUE) {
    all.se <- c(list(sce), as.list(altExps(sce)))

    args <- list(
        assays=.unsplit_assays(all.se),
...
.unsplit_assays <- function(all.se) {
    ...
    for (a in all.assays) {
        ...
        for (s in seq_along(all.se)) {
            ...
            if (a %in% assayNames(cur.se)) {
                current[[s]] <- DelayedArray(assay(cur.se, a))
            } else {
                current[[s]] <- ConstantMatrix(dim(cur.se), value=NA)
            }
        }
